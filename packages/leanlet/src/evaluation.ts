...
>(
  cases: readonly ClassificationEvaluationCase<Input, Label>[],
  run: (input: Input) => Promise<LeanletResult<Output>>,
  labelOf: (output: Output) => Label,
): Promise<ClassificationEvaluationMetrics<Label>> {
  const labels = new Map<
    Label,
    { support: number; predicted: number; correct: number }
  >();
  ...
  const byLabel = {} as ClassificationEvaluationMetrics<Label>['labels'];
  let f1Total = 0;
