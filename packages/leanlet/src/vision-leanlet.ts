...
type PendingRequest = { resolve: (result: CategoryResult) => void; reject: (error: Error) => void };

export class VisionLeanlet {
  private worker!: Worker;
  private listeners = new Set<Listener>();
  private pending = new Map<string, PendingRequest>();
  private options: Required<Omit<VisionLeanletOptions, 'workerUrl'>> & { workerUrl?: URL };

  constructor(options: VisionLeanletOptions = {}) {
    if (typeof Worker === 'undefined') throw new Error('VisionLeanlet requires a browser with Web Worker support.');
    this.options = {
  ...
  classify(file: Blob, options: ClassifyOptions = {}) {
    ...
    const request = new Promise<CategoryResult>((resolve, reject) => this.pending.set(requestId, { resolve, reject }));
