...
function loadModel() {
    ...
    if (definition.task === 'zero-shot-image-classification') {
        const textOptions = { dtype: definition.textDtype ?? 'q8', device: 'wasm', progress_callback: onProgress };
        const visionOptions = { dtype: definition.visionDtype ?? 'q8', device: 'wasm', progress_callback: onProgress };
...
async function classify(image, allowed) {
    ...
    if (runtime.kind === 'mobileclip') {
        ...
        if (textCache?.key !== cacheKey) {
            ...
            textCache = { key: cacheKey, embeddings: textEmbeds.normalize().tolist() };
        }
